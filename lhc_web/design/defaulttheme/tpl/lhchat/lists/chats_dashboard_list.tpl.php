<div class="row">
    <div class="col chats-column border-right pr-0 pl-2">
        ...
        <div class="overflow-auto" style="max-height: 300px;">
            <div class="row chat-list-row border-bottom row" ng-repeat="chat in my_chats.list track by chat.id" ng-click="lhc.startChatDashboard(chat.id,chat.last_msg_id)" ng-class="{'active-chat-row' : chat.id == lhc.current_chat_id,'user-away-row': chat.user_status_front == 2, 'user-online-row': chat.user_status_front == 0}">
                <div  class="col-8 col-xs-5 col-sm-5 col-md-5 col-lg-4 col-xl-2 align-self-center p-1 ml-1">
                    <i><img ng-class="{'icon-svg-online' : lhc.chatMetaData[chat.id]['ucs'] == 0,'icon-svg-away' : lhc.chatMetaData[chat.id]['ucs'] == 2,'icon-svg-pageview' : lhc.chatMetaData[chat.id]['ucs'] == 3}"  class = "img-fluid align-self-center rounded  icon-svg-status" src="{{lhc.getIcon(chat.id)}}"/></i>
                </div>
                <div class="col-2 col-xs-2 col-sm-2 col-md-2 col-lg-6 col-xl-8">
                    <i id="msg-send-status-{{chat.id}}" ng-class="{'icon-user-offline' : lhc.chatMetaData[chat.id]['um'] == 1}" title="Last message send status" class="pt-3 pt-lg-1 pt-md-3 pt-sm-3 fs12 material-icons icon-user-online float-right">send</i>
                    ...
                    <div class="fs13">
                        <div  class="fs11 pt-2 text-secondary d-none d-lg-inline">
                                ...
                                <span title="{{chat.id}}" class="fs12 d-none d-lg-inline" ng-class="{'chat-pending font-weight-bold': !chat.status, 'chat-unread': chat.status == 2,'font-weight-bold' : lhc.chatMetaData[chat.id]['mn'] > 0}">{{lhc.truncateScope(chat.nick,10)}}</span>
